...
const VERCEL_CMD = isWindows ? 'vercel.cmd' : 'vercel';
// Whitelist of allowed command names to prevent command injection
const ALLOWED_COMMANDS = new Set(['vercel', 'vercel.cmd', 'npm', 'pnpm', 'yarn']);
function log(msg) {
  console.error(msg);
}
function commandExists(cmd) {
  ...
  try {
    if (isWindows) {
      ...
      const result = spawnSync('sh', ['-c', `command -v "local"`, '--', cmd], { stdio: 'ignore' });
      return result.status === 0;
    }
  } catch {
    return false;
...
function checkVercelInstalled() {
  if (!commandExists(VERCEL_CMD)) {
    log('Error: Vercel CLI is not installed');
    log('Please run install script first to install Vercel CLI');
    process.exit(1);
  }
  const version = getCommandOutput(VERCEL_CMD, ['--version']) || 'unknown';
  log(`Vercel CLI version: ${version}`);
}
function checkLoginStatus() {
  log('Checking login status...');
  try {
    const result = spawnSync(VERCEL_CMD, ['whoami'], { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], shell: isWindows });
